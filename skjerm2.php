...
$Q_skjermer = mysql_query("select * from `skjermer` order by `skjerm_navn`");
$skjermer = array();
$slides_max = 0;
while($skjerm = mysql_fetch_assoc($Q_skjermer))
{
	$skjermer[$skjerm['skjerm_id']] = $skjerm;
	...
	$skjermer[$skjerm['skjerm_id']]['slides_nu'] = $slides_nu;
	$skjermer[$skjerm['skjerm_id']]['slides_andre'] = $slides_andre;
	
	if($skjerm['skjerm_slidenr'] > $slides_max)
		$slides_max = $skjerm['skjerm_slidenr'];
}

$smarty->assign('skjermer', $skjermer);
$smarty->assign('slides_max', $slides_max);
