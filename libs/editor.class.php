...
class editor {
	...
	function makeNewField ($var, $name, $type, $options = array()) {
		if($var == '')
		{
			$this->error_code = ''; // Var for field is not set
			return FALSE;
		}
		if($name == '')
			$name = $var;
		...
		switch ($type)
		{
			...
			case 'textarea':
				$this->vars[$var]['type'] = $type;
				$this->vars[$var]['processor'] = 'text';
				$this->vars[$var]['checker'] = 'text';
				break;
			default:
				$this->error_code = ''; // Type of field is invalid.
				return FALSE;
				break;
		}
		...
		if(isset($options['rows']) && is_numeric($options['rows']))
			$this->vars[$var]['rows'] = $options['rows'];
		if(isset($options['cols']) && is_numeric($options['cols']))
			$this->vars[$var]['cols'] = $options['cols'];
	...
	function setFieldProcessor ($var, $functionName) {
		...
		{
			$this->error_code = ''; // Field does not exist.
			return FALSE;
		}
		if($functionName == '')
		{
			$this->error_code = ''; // Function name is not set.
			return FALSE;
		}
	...
	function input ($input) {
		if(!is_array($input))
		{
			$this->error_code = ''; // Input is not array
			return FALSE;
		}
		...
		if($error_found)
		{
			$this->error_code = ''; // Error was found in input
			return FALSE;
		}
		else
		{
	...
	function processInput_text ($var, $input) {
		return $this->slashes(htmlspecialchars($input,ENT_QUOTES));
	}
	...
	function getDB () {
		if($this->id != '')
		{
			$QUERY = mysql_query("select * from `".$this->db_table."` where `".$this->db_field_id."` = '".$this->id."'");
			if(mysql_num_rows($QUERY))
			{
				foreach ($this->vars as $var)
				{
					...
					{
						if($var['type'] == 'checkbox')
							$this->vars[$var['var']]['value_array'] = $this->splittIDs(mysql_result($QUERY, 0, $var['var']));
						else
	...
	function printEditor () {
		...
		if($this->showid && $this->id != '') {
			echo "\t<tr>\n\t\t<td>"._('ID')."</td>\n\t\t<td><input size=\"4\" type=\"text\" disabled=\"disabled\" value=\"".$this->id."\"></td>\n\t</tr>\n";
		}
		...
		foreach ($this->vars as $var)
		{
			...
			switch ($var['type'])
			{
				...
				case 'textarea':
					...
					echo '<textarea name="'.$var['var'].'"'.$rows.$cols.$onchange.$onsubmit.$onreset.$onselect.$onblur.$onfocus.$disabled.$id.'>'.$var['value'].'</textarea>';
					break;
	...
	function performDBquery ()
	{
		...
		foreach ($this->vars as $var)
		{
			if($var['DBQueryPerform'])
			{
				...
				{
					$fields[$var['var']] = $this->splittalize($var['value_array']);
				}
				else
		...
		if($this->id != '')
		{
			...
			$SQL .= " WHERE `".$this->db_field_id."` = '".$this->id."' LIMIT 1 ;";
	...
	function splittalize ($inputarray, $indexkeep = false)
	{
		$return = '';
